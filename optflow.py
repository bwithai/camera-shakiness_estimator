...
class TranslationTransformationGetter(TransformationGetter):
    ...
    def __call__(
        ...
        # Indicating mode of optical flow
...
class MotionEstimator:
    ...
    def update(
    ...
    ) -> tuple[CoordinatesTransformation, ndarray, float | ndarray]:
        """
        ...
        # ----------------------------------------------------------------------------------------------------
        motion_vector = curr_pts - self.prev_pts
        trajectory = np.cumsum(motion_vector, axis=0)
        motion_magnitude = np.linalg.norm(motion_vector, axis=1)
        # motion_direction = np.mean(motion_vector, axis=0)

        # Calculate the average magnitude and displacement
        self.average_magnitude = np.mean(motion_magnitude)
        self.displacement = np.linalg.norm(trajectory[-1] - trajectory[0])
        # ----------------------------------------------------------------------------------------------------
        ...
        if self.draw_flow:
            ...
            motion_vector = curr_pts - self.prev_pts
            motion_direction = np.mean(motion_vector, axis=0)
            for (curr, prev, motion) in zip(curr_pts, self.prev_pts, motion_vector):
                ...
                # c = tuple(curr.astype(int).ravel())
                # p = tuple(prev.astype(int).ravel())
                # m = tuple(motion.astype(int).ravel())
                # # Draw the motion vector with green color
                # cv2.arrowedLine(frame, c, p, (0, 255, 0), 2, tipLength=0.5)
                # # Draw the extended motion vector with red color
                # cv2.arrowedLine(frame, c, (c[0] + m[0], c[1] + m[1]), (0, 0, 255), 2, tipLength=0.5)

        if len(self.prev_pts) < 4 or len(curr_pts) < 4:
            error = {
                "Error": "The input arrays should have at least 4 corresponding point sets to calculate Homography in "
                         "function 'findHomography'",
                "Message": "The video contains high shake, making it difficult to accurately calculate the "
                           "transformation. Please stabilize the video or choose another video with less shake."
            }

            error_json = json.dumps(error, indent=2)
            print("\n",error_json)
            exit()
        ...
        return coord_transformations, self.average_magnitude, self.displacement
